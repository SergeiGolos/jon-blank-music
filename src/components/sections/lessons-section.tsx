      ...
      <div class="mb-8 rounded-3xl border border-white/20 bg-white/10 p-8 shadow-2xl backdrop-blur-lg">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
          {[
            ...
            const dayLessons = lessonsData.filter(
              (lesson) => lesson.day === day && lesson.available,
            );
            return (
              <div key={day} class="rounded-xl bg-white/5 p-4">
                <h4 class="mb-4 text-center text-lg font-semibold text-white">
                  {day}
                </h4>
                <div class="space-y-2">
                  {dayLessons.length > 0 ? (
                    dayLessons.map((lesson) => (
                      <button
                        ...
                        class="w-full cursor-pointer rounded-lg border border-white/20 bg-white/10 p-3 text-center transition-all duration-200 hover:scale-105 hover:border-amber-400/50 hover:bg-white/20 hover:shadow-lg active:scale-95 active:bg-white/30"
                      >
                        <div class="font-medium text-amber-300">
                          {lesson.time}
                        </div>
                        <div class="text-sm text-green-400">Available</div>
                      </button>
                    ))
                  ) : (
                    <div class="py-3 text-center text-white/60">No slots</div>
                  )}
                </div>
              </div>
            );
